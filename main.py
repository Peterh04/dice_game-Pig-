import random


def roll():
    min_value = 1
    max_Value = 6
    roll = random.randint(min_value, max_Value)
    
    return roll

while True:
    players = input('Enter number of players (2 -4): ')
    if players.isdigit():
        players = int(players)
        if players >=2 and players <= 4:
            break
        else:
            print('Only 2-4 players allowed')
            
max_score = 50
player_scores =[0 for i in range(players)]
 
while max(player_scores) < max_score:
  for play_idx in range(players):
    
    
    print("\nPlayer number", play_idx + 1, "turn has just started!")
    print("Your total score is: \n" , player_scores[play_idx])
    current_score = 0
    
    while True:
        should_roll = input('Would you like to roll (y)? ')
        if should_roll.lower() != "y":
            break       
        
        value = roll()
        if value == 1:
            print("You rolled a 1! Turn done!")
            current_score = 0
            break
        
        else:
            current_score += value
            print("You rolled a: ", value)
            
        print("Your score is:", current_score) 
        
    player_scores[play_idx] += current_score 
    
    print("Your total score is: ", player_scores[play_idx])
else:
    print("Player: ", play_idx, "You won!")  

 
max_score = max(player_scores)
winin_idx = player_scores.index(max_score)
print("player number", winin_idx +1, "is the winner", max_score)  

               
     